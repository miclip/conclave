 ...
 * applies to every implementer seat (bin/conclave.ts:2195-2198) and project config still carries
 * no per-agent launch args at all (`launchArgsFor` only returns bypass flags,
 * src/config/project.ts:425-428) -- but distinct args per seat ARE now expressible on the
 ...
 * still the production ones: `runReport` is what `bin/conclave.ts:2523` prints, and the status
