 ...
 * applies to every implementer seat (bin/conclave.ts:2253-2256) and project config still carries
 * no per-agent launch args at all (`launchArgsFor` only returns bypass flags,
 * src/config/project.ts:445-448) -- but distinct args per seat ARE now expressible on the
 ...
 * still the production ones: `runReport` is what `bin/conclave.ts:2581` prints, and the status
