...
export interface ResolutionConfig {
   ...
   * configured)` (`src/relay/relay.ts:1858`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:2446`), and the two pause sites below it are reachable only
        // WITH rotation configured (`src/relay/relay.ts:2440-2445` ends the run instead when
