...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:536`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:1860`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:2448`), and the two pause sites below it are reachable only
        // WITH rotation configured (`src/relay/relay.ts:2442-2447` ends the run instead when
