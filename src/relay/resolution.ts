...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:877`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:3029`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:340`), which is where a
        ...
        // (`src/relay/relay.ts:4597-4599`), which is what makes the derivation mean anything: with
