...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:870`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:2543`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:325`), which is where a
        ...
        // (`src/relay/relay.ts:3669-3671`), which is what makes the derivation mean anything: with
