...
export interface ResolutionConfig {
   ...
   * configured)` (`src/relay/relay.ts:3535`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:349`), which is where a
        ...
        // (`src/relay/relay.ts:5132`), which is what makes the derivation mean anything: with
