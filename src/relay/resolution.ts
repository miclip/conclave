...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:688`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:2389`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:3515-3517`), which is what makes the derivation mean anything: with
