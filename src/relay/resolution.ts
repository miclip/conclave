...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:534`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:1823`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:2411`), and the two pause sites below it are reachable only
        // WITH rotation configured (`src/relay/relay.ts:2405-2410` ends the run instead when
