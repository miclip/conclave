...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:647`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:2349`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:3425-3427`), which is what makes the derivation mean anything: with
