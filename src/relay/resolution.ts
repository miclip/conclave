...
export interface ResolutionConfig {
   ...
   * configured)` (`src/relay/relay.ts:3071`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:341`), which is where a
        ...
        // (`src/relay/relay.ts:4657`), which is what makes the derivation mean anything: with
