...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:939`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:3129`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:4715`), which is what makes the derivation mean anything: with
