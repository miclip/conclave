...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:1032`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:3150`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:342`), which is where a
        ...
        // (`src/relay/relay.ts:4736`), which is what makes the derivation mean anything: with
