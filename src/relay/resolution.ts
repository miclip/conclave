...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:1188`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:4015`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:5973`), which is what makes the derivation mean anything: with
