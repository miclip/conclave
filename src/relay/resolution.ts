...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:663`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:2367`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:324`), which is where a
        ...
        // (`src/relay/relay.ts:3443-3445`), which is what makes the derivation mean anything: with
