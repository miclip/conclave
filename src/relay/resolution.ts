...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:1144`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:3852`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:5575`), which is what makes the derivation mean anything: with
