...
export interface ResolutionConfig {
   ...
   * (`src/repl/session.ts:472-473`) and the run reports `rotation: NOT ARMED (no checks
   * configured)` (`src/relay/relay.ts:1171`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // (`src/relay/relay.ts:1701`), and the two pause sites below it are reachable only
        // WITH rotation configured (`src/relay/relay.ts:1695-1699` ends the run instead when
