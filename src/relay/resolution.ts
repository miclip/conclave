...
export interface ResolutionConfig {
   ...
   * configured)` (`src/relay/relay.ts:2988`).
   */
  ...
  switch (subject.reason) {
    case 'rotation_candidate':
      return {
        ...
        // resolved for the seat in `rotationFor` (`src/relay/relay.ts:333`), which is where a
        ...
        // (`src/relay/relay.ts:4484-4486`), which is what makes the derivation mean anything: with
