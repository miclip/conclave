...
const DECLARED: Record<string, string> = {
  ...
  'status.outcome.reason gains the two control-channel endings (#266)':
    ...
    'listing spreads the whole status document (bin/conclave.ts:1431), so an outcome written by ' +
 ...
 * call sites: the report is what `bin/conclave.ts:2581` prints, and the status record is what
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:2479-2481.
