...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7550.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:7167, src/relay/relay.ts:7687), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7284), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7601) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:7612), which is what ' +
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4442), where the
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2443-2449.
  // The cwd getter simply returns the option: src/relay/relay.ts:2144-2146.
  ...
  // worktreePaths, and src/relay/relay.ts:3351-3352, :3949 and :6383 read it.
