...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:5168.',
  'status.pause.resolution':
    ...
    'classification (src/relay/run.ts:215-229). Nothing an existing consumer read changed value.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:4921, src/relay/relay.ts:5305), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:5020), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:5217) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:5231), which is what ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    '(src/relay/run.ts:523). There was one pause, read twice — the same defect by a shorter ' +
 ...
 * call sites: the report is what `bin/conclave.ts:1342` prints, and the status record is what
...
async function pausedStatusDocument(reason: PauseReason): Promise<unknown> {
  ...
  const recording = recordSession(relay, {
    ...
    front: 'session',
    startedAt: Date.now(),
    // Passed because the console always passes one (src/repl/session.ts:826), so the status
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2340, :2568 and :4429 read it.
