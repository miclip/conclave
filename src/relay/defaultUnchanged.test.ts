...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:4474.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:4227, src/relay/relay.ts:4613), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:4326), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:4523) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:4536), which is what ' +
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:2727), where the
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2228, :2325 and :3757 read it.
