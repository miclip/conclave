...
const DECLARED: Record<string, string> = {
  ...
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7963.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:7546, src/relay/relay.ts:8100), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7681), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:8014) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:8025), which is what ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    '(src/relay/run.ts:872). There was one pause, read twice — the same defect by a shorter ' +
 ...
 * call sites: the report is what `bin/conclave.ts:1949` prints, and the status record is what
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4596), where the
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:1850-1852.
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2510-2516.
  // The cwd getter simply returns the option: src/relay/relay.ts:2213-2215.
  ...
  // worktreePaths, and src/relay/relay.ts:3503-3504, :4101 and :6676 read it.
