...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:4866.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:4619, src/relay/relay.ts:5003), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:4718), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:4915) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:4929), which is what ' +
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2273, :2390 and :4148 read it.
