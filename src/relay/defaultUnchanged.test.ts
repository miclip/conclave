...
const DECLARED: Record<string, string> = {
  ...
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:8946.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:8363, src/relay/relay.ts:9083), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:8664), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:8997) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:9008), which is what ' +
  ...
  'status.outcome.reason gains the two control-channel endings (#266)':
    ...
    'listing spreads the whole status document (bin/conclave.ts:1425), so an outcome written by ' +
 ...
 * call sites: the report is what `bin/conclave.ts:2551` prints, and the status record is what
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:5260), where the
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:2449-2451.
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2759-2765.
  ...
  // worktreePaths, and src/relay/relay.ts:3906-3907, :4626 and :7469 read it.
