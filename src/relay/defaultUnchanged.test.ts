...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:6974.',
  'status.pause.resolution':
    ...
    'classification (src/relay/run.ts:229-243). Nothing an existing consumer read changed value.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:6595, src/relay/relay.ts:7111), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:6712), resuming ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    '(src/relay/run.ts:747). There was one pause, read twice — the same defect by a shorter ' +
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2928-2929, :3501 and :5857 read it.
