...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:4873.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:4626, src/relay/relay.ts:5010), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:4725), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:4922) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:4936), which is what ' +
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2280, :2397 and :4155 read it.
