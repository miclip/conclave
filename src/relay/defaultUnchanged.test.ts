...
interface CreateRecord {
   ...
   * for its whole life (src/repl/session.ts:278-290). This is the observation that would have
   * caught it.
   */
...
const DECLARED: Record<string, string> = {
  ...
  'status.pause.resolution':
    ...
    'classification (src/relay/run.ts:315). Nothing an existing consumer read changed value.',
  ...
  'a mixed CPU sample is reported as mixed rather than as a working child (#83)':
    ...
    'file was relaxed; one citation moved with the code it pins (src/repl/session.ts:1606). ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    'has at src/repl/session.ts:362) and livenessRefreshMs/livenessRefreshLimit. No CLI flag ' +
    ...
    '(src/relay/run.ts:893). There was one pause, read twice — the same defect by a shorter ' +
  ...
  '--silence-timeout on both front-ends':
    ...
    'example of what that costs (src/repl/session.ts:278-290).',
 ...
 * call sites: the report is what `bin/conclave.ts:1969` prints, and the status record is what
 ...
 * `set('paused', { pause })` is the same call the console makes at src/repl/session.ts:1606
...
async function pausedStatusDocument(t: TestContext, reason: PauseReason): Promise<unknown> {
  ...
  const recording = recordSession(relay, {
    ...
    front: 'session',
    startedAt: Date.now(),
    // Passed because the console always passes one (src/repl/session.ts:1235), so the status
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:1870-1872.
 ...
 *     its whole life (src/repl/session.ts:278-290), and a text-level pin cannot tell the
 *     difference.
