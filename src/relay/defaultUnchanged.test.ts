...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7345.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:6962, src/relay/relay.ts:7482), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7079), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7396) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:7407), which is what ' +
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4276), where the
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // The cwd getter simply returns the option: src/relay/relay.ts:2085-2087.
  ...
  // worktreePaths, and src/relay/relay.ts:3194-3195, :3792 and :6178 read it.
