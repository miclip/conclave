...
const DECLARED: Record<string, string> = {
  ...
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7947.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:7530, src/relay/relay.ts:8084), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7665), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7998) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:8009), which is what ' +
  ...
  'a mixed CPU sample is reported as mixed rather than as a working child (#83)':
    ...
    'file was relaxed; one citation moved with the code it pins (src/repl/session.ts:1591). ' +
 ...
 * `set('paused', { pause })` is the same call the console makes at src/repl/session.ts:1591
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4580), where the
...
async function pausedStatusDocument(t: TestContext, reason: PauseReason): Promise<unknown> {
  ...
  const recording = recordSession(relay, {
    ...
    front: 'session',
    startedAt: Date.now(),
    // Passed because the console always passes one (src/repl/session.ts:1220), so the status
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2494-2500.
  // The cwd getter simply returns the option: src/relay/relay.ts:2197-2199.
  ...
  // worktreePaths, and src/relay/relay.ts:3487-3488, :4085 and :6660 read it.
