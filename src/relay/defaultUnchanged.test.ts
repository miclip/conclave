...
interface CreateRecord {
   ...
   * for its whole life (src/repl/session.ts:291-303). This is the observation that would have
   * caught it.
   */
...
const DECLARED: Record<string, string> = {
  'deny-only project configuration for capabilities and commands':
    'A project may switch off a declared capability or an allowed slash command in ' +
    '.conclave/config.json (#203). Deny-only: `true` is refused at read, because a declaration ' +
    'is something read off the installed CLI and a command allowance was argued against the ' +
    'continuity, parser and report guarantees conclave keeps. A project with NEITHER map is ' +
    'unchanged in every respect, and that is asserted rather than assumed: `denialsFrom` ' +
    'returns undefined, both front-ends spread the key in only when it is present, and both ' +
    'narrowing functions return the object they were given. The divergence is therefore visible ' +
    'only to a project that opted into it.',
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:8788.',
  'status.pause.resolution':
    ...
    'and the run configuration (src/relay/resolution.ts:199), and a request whose authority is ' +
    ...
    'classification (src/relay/run.ts:416). Nothing an existing consumer read changed value.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:8205, src/relay/relay.ts:8925), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:8506), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:8839) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:8850), which is what ' +
  ...
  'a mixed CPU sample is reported as mixed rather than as a working child (#83)':
    ...
    'file was relaxed; one citation moved with the code it pins (src/repl/session.ts:1757). ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    '(src/relay/run.ts:1145). There was one pause, read twice — the same defect by a shorter ' +
  ...
  '--ready-timeout on both front-ends':
    'The optional flag surface grows by one on each command: --ready-timeout SECONDS, which ' +
    'bounds how long a seat may take to report SessionStart before the adapter gives up. The ' +
    'window itself is not new -- `readyTimeoutMs` has been read by both pty adapters since they ' +
    'existed, defaulting to sixty seconds -- and what is new is that anything can set it. It was ' +
    'declared in registry/types.ts, read in builtin.ts, and constructed nowhere: every ctx was ' +
    'built as { cwd, watchdogMs, idleMs }. So the readiness failure told an operator to "raise ' +
    'readyTimeoutMs" and no flag, env var or config key did that. #271 is the incident: a cold CI ' +
    'runner with a freshly installed CLI comes up in onboarding, the first hook lands well past a ' +
    'minute, and the run ended on a window that could not be widened. Optional and valued on ' +
    'both, with no default, so a run that does not pass it is measured exactly as it was before ' +
    'the flag existed.',
  '--silence-timeout on both front-ends':
    ...
    'example of what that costs (src/repl/session.ts:291-303).',
  ...
  'the advisor is told what its writing seats can be instructed to do (#192)':
    'The advisor\u2019s opening prompt gains a conditional block naming what each WRITING seat can ' +
    'be INSTRUCTED to do \u2014 subagents, background tasks, session backgrounding, async prompts, ' +
    'bounded iteration, an autonomous loop \u2014 resolved per seat through the registry definition ' +
    'that seat was filled from, never from an agent id. THIS ONE IS DIFFERENT FROM EVERY OTHER ' +
    'CONDITIONAL BLOCK IN THAT BRIEFING, and that is the reason it is written down here rather ' +
    'than passed over as additive. MULTI_SEAT_BRIEFING is off unless a run has two seats; the ' +
    'reviewer block is off unless a run declares one; the role block is off unless a role is ' +
    'described. All three are off on a default run. THIS ONE IS ON: the default implementer is ' +
    '`claude` (bin/conclave.ts, `flag(\'implementer\', \'claude\')`), CLAUDE_AGENT declares four ' +
    'supported capabilities, and so an ordinary `conclave session` now sends its advisor roughly ' +
    'a thousand characters it did not send before. The guards in this file do not catch it ' +
    'because they build their own registry, whose synthetic definitions declare nothing \u2014 ' +
    'which is exactly the blind spot the multi-seat briefing guard was added to close, one field ' +
    'along. WHAT REMAINS TRUE: a run whose seated agents declare nothing is byte-identical, ' +
    'including a declaration that is empty or supported:false throughout, and that is pinned in ' +
    'src/relay/capabilityBriefing.test.ts. WHAT AN OPERATOR SHOULD WEIGH: ' +
    'spikes/experiments/04-complaint-as-signal.md says the briefing must NOT change ' +
  ...
  'a rotation candidate says which class raised it':
    'A `rotation_candidate` pause now carries `candidate.assessedAs` -- `degraded`, ' +
    '`corroborated`, `unbacked` or `nothing` -- beside the prose in `detail` (#247). Additive, ' +
    'and on that reason alone. `assess` returns the class and the relay already carried it WHOLE ' +
    'internally, with a comment saying `degraded` and `corroborated` are two different questions ' +
    'to put to an operator and must not be folded into a boolean. Every surface a reader could ' +
    'reach then folded them anyway: `detail` says "compacted" for both, and ' +
    '`RotationRecord.reason` copies that prose, so the flattening outlived the run. The ' +
    'distinction is the design\'s own -- `degradation.ts` holds that a model may compact without ' +
    'noticing, or notice and not say, so a complaint is corroborating evidence and never a ' +
  ...
  '--checkpoint arms a decision point, which relay cannot hold':
    'THE DEFAULT RUN DOES NOT CHANGE, and every arm of that is pinned below rather than ' +
    'asserted here. With --checkpoint absent the console calls relay.start(goal) with ONE ' +
    'argument exactly as it always did, no checkpoint key reaches the status document, the ' +
    "advisor's briefing is byte-identical (the conditional block is gated on the armed " +
    'checkpoint, the same gate MULTI_SEAT_BRIEFING uses), and readMilestone is never called on ' +
    'a reply — so an advisor that writes MILESTONE: on an unarmed run has its reply dispatched ' +
    'as the ordinary instruction it is. WHAT IS NEW: a PauseReason, operator_checkpoint, whose ' +
    'pause subtree is pinned below and which is unreachable without the operator arming one, ' +
    'for the same reason merge_blocked is unreachable without a second implementer seat — ' +
    'except that this one needs no extra seat, only the operator having asked in advance. ' +
    'A status key, checkpoint, present exactly when one was armed and PERSISTING past the ' +
    'release so that a run which reached its checkpoint and one which ended having never ' +
    'signalled are distinguishable afterwards. Its state is THREE-VALUED — armed, signalled, ' +
    'continued — because the advisor reporting a milestone and the operator accepting it are ' +
    'different facts: an abort at the checkpoint pause keeps signalled and makes no ' +
    'not-reached claim, and only a run that ended with nothing signalled at all says the ' +
    'checkpoint was missed, appended in #end beside the red-tree sentence and by the same ' +
    'construction, so a missing signal is never silent. Each arming carries a per-run ' +
    'generation (never the routing log seq, which orders messages), the operator_checkpoint ' +
    'pause carries checkpoint {generation, milestone} of its own, and a continuation resolves ' +
    'only the generation its pause names — so arming a second checkpoint while the first is ' +
    'paused leaves the second armed and unstamped rather than silently spent. Relay.start ' +
    'resets the whole record on EVERY run, so a second run inherits neither a spent checkpoint ' +
    'nor one an abort left armed. THE FLAG IS SESSION-ONLY, which is the ' +
    'first divergence on this surface running session-to-relay rather than the other way: a ' +
    'checkpoint arms a pause the operator answers, and relay escalates and ENDS the run at ' +
    'every pause (Relay#halt, the !handle branch), so relay --checkpoint would take the run it ' +
    'was meant to interrupt and end it at the milestone with nobody able to release it. Wiring ' +
  ...
  'status.outcome.reason gains the two control-channel endings (#266)':
    ...
    'listing spreads the whole status document (bin/conclave.ts:1265), so an outcome written by ' +
 ...
 * call sites: the report is what `bin/conclave.ts:2295` prints, and the status record is what
 ...
 * `set('paused', { pause })` is the same call the console makes at src/repl/session.ts:1757
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:5110), where the
...
async function provoke(
  ...
  const scripts: Record<PauseReason, { advisor: string[]; impl: string[] }> = {
    ...
    // The advisor's FIRST reply is the signal, and the checkpoint it reports is armed below --
    // this is the one condition in the table that is provoked by arming something rather than by
    // making something go wrong. `DONE` follows it so the run has somewhere to go on release.
    operator_checkpoint: {
      advisor: ['MILESTONE: the first thing is done.', 'DONE'],
      impl: ['ack'],
    },
  }
  ...
  const run = relay.start(
    reason === 'implementer_unanswered' ? 'Add the endpoint.' : 'Keep the work moving.',
    // Armed at `start` for the one reason that needs it, and for no other. A checkpoint is the
    // operator asking in advance, so there is no way to reach this pause on a run nobody asked
    // -- which is exactly what makes the entry a declared departure rather than a default
    // document. Every other variant passes no second argument at all, so the call they make is
    // the one they have always made.
    reason === 'operator_checkpoint' ? { checkpoint: 'the first thing is done' } : {},
  )
...
async function pausedStatusDocument(t: TestContext, reason: PauseReason): Promise<unknown> {
  ...
  const recording = recordSession(relay, {
    ...
    front: 'session',
    startedAt: Date.now(),
    // Passed because the console always passes one (src/repl/session.ts:1334), so the status
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:2193-2195.
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2721-2727.
  // The cwd getter simply returns the option: src/relay/relay.ts:2402-2404.
  ...
  assert.match(
    RELAY,
    /const ctx = \{ cwd, watchdogMs: this\.#opts\.turnWatchdogMs, idleMs: this\.#opts\.silenceWatchdogMs, readyTimeoutMs: this\.#opts\.readyTimeoutMs \}\s*const session = await this\.#opts\.registry\.createParticipant\(spec, ctx\)/,
  ...
  assert.match(
    RELAY,
    /const ctx = \{ cwd: root, watchdogMs: this\.#opts\.turnWatchdogMs, idleMs: this\.#opts\.silenceWatchdogMs, readyTimeoutMs: this\.#opts\.readyTimeoutMs \}\s*const session = await this\.#opts\.registry\.createParticipant\(spec, ctx\)/,
  ...
  // worktreePaths, and src/relay/relay.ts:3867-3868, :4587 and :7311 read it.
...
test('both flag helpers fall back when the flag is absent, and each block reads exactly these flags', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      'max-turns',
      'operator',
      // The declared addition; see DECLARED['--ready-timeout on both front-ends'].
  ...
  assert.deepEqual(
    ...
    [
      'advisor',
      'advisor-args',
      // SESSION ONLY, and the first entry on this surface that relay does not also have --
      // every other declared addition landed on both. See
      // DECLARED['--checkpoint arms a decision point, which relay cannot hold'] and the
      // matching entry in `frontEndParity.test.ts`: relay ends the run at every pause, so a
      // checkpoint there would stop the run it was meant to interrupt with nobody able to
      // release it. Valued and optional, with no default, so a console run that does not pass
      // it calls `relay.start(goal)` with one argument exactly as it always did.
      'checkpoint',
      'checks',
      'checks-informational',
      'checks-unrelated',
 ...
 *     its whole life (src/repl/session.ts:291-303), and a text-level pin cannot tell the
 *     difference.
...
test('the default relay --json report emits exactly these keys at every depth', async (t) => {
  ...
  assert.deepEqual(
    shapeOf(report),
    {
      ...
      '': 'activeMs, build, cwd, deadlines, durationMs, endedAt, flags, forces, goal, messages, operator, outcome, participants, pausedMs, restricted, rotation, schema, startedAt, subagents, supersededFlags',
...
const PAUSED_SUBTREE: Record<PauseReason, Record<string, string>> = {
  rotation_candidate: {
    // `candidate` is on THIS reason and no other, which is what the arm-by-arm shape is for: it
    // says which class of degradation raised the pause, as data beside the prose in `detail`
    // (#247). See DECLARED['a rotation candidate says which class raised it'].
    'pause.candidate': 'assessedAs',
    pause: 'at, atSeq, candidate, detail, evidence, options, reason, resolution',
  ...
  // A DECLARED DEPARTURE, and the third kind in this table.
  //
  // The other two departures are structural: `merge_blocked` and `review_blocked` need seats a
  // default run does not have, and `rotation_candidate` needs `--checks`. This one needs neither
  // -- the run below is a bare default run, one advisor and one implementer, no flags -- but it
  // cannot be reached without the operator having ARMED a checkpoint before the run began, which
  // no default invocation does. So the departure is not in the run's configuration; it is that
  // the operator asked for this pause in advance, which is the whole of what the reason means.
  //
  // The subtree is nevertheless the same one `operator_requested` emits, key for key, and that
  // is the assertion rather than a coincidence worth relaxing. Both are conclave-scoped operator
  // suspensions carrying nothing but the standard block: no `verdictOf`, because no verdict is
  // in question -- the advisor's turn completed and its reply was whole; no `candidate`, because
  // nothing was assessed; no `conflict`, because there are not two sides. A build that started
  // emitting any of them here would be attaching evidence to a pause that rests on none.
  operator_checkpoint: {
    pause: 'at, atSeq, checkpoint, detail, evidence, options, reason, resolution',
    'pause.resolution': 'authority, reason, scope',
    'pause.resolution.scope': 'kind',
    // The ONLY document in this corpus carrying a `checkpoint` block, and the only one that
    // could: the key is present exactly when the operator armed one, so its absence from the
    // other eight documents is what makes its presence here a decision rather than a build that
    // reports it unconditionally. See
    // DECLARED['--checkpoint arms a decision point, which relay cannot hold'].
    //
    // This restates the root key list rather than inheriting PAUSED_COMMON's for the same
    // reason `merge_blocked` restates it for `targeting`: a common pin that included
    // `checkpoint` would let it appear on every document, and one that omitted it would let it
    // vanish from this one.
    '': 'abandoned, alive, blocked, build, ceilings, checkpoint, cwd, deadlines, eventsPath, forces, front, goal, id, logPath, messages, operator, participants, pause, pid, progress, rotations, schema, startedAt, state, updatedAt',
    // FIVE keys, and `continuedAt` is the sixth by its ABSENCE. This document is captured at the
    // checkpoint's own pause: the advisor has signalled -- so `state` is `signalled` and
    // `signalledAt` is stamped -- and the operator has been asked and has not answered, so there
    // is no continuation to timestamp. A build that emitted `continuedAt` here would be
    // reporting an acceptance nobody has given yet; that shape belongs to a document captured
    // after a `/continue` and is pinned in src/repl/checkpoint.test.ts against a real run
    // rather than here, because a paused corpus cannot reach it.
    checkpoint: 'at, generation, milestone, signalledAt, state',
    // WHICH checkpoint this pause is about, as data. Present on this reason and no other -- the
    // same arm-by-arm rule `pause.candidate` follows -- and it exists so a reader can line the
    // pause up against the top-level block after a paused re-arm, when the two are deliberately
    // different checkpoints. See DECLARED['--checkpoint arms a decision point, which relay
    // cannot hold'].
    'pause.checkpoint': 'generation, milestone',
  },
...
test('a project that denies nothing reaches Relay.start with no denial key on either front-end', () => {
  // THE IDENTITY RULE, pinned at the only place it can be broken cheaply. `denialsFrom` returns
  // undefined for a project with no maps, and each front-end spreads `denied` only when it is
  // present -- so the options object a default run builds is the one it built before #203
  // existed, rather than one carrying an empty denial that behaves the same.
  //
  // Read off the SOURCE because that is what this guard protects: a run asserting behaviour
  // would pass just as well against `denied: { capabilities: [], commands: [] }`, which is the
  // exact shape this rule exists to keep out of a default run.
  const SESSION = readFileSync(join(import.meta.dirname, '..', 'repl', 'session.ts'), 'utf8')
  const conditional = '...((d) => (d ? { denied: d } : {}))(denialsFrom(projectConfig))'
  for (const [where, source] of [['bin/conclave.ts', CLI], ['src/repl/session.ts', SESSION]] as const) {
    assert.ok(
      source.includes(conditional),
      `${where} must spread 'denied' only when the project denies something, so a project with ` +
        `no maps passes no key at all`,
    )
  }
})
