...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:5152.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:4905, src/relay/relay.ts:5289), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:5004), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:5201) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:5215), which is what ' +
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // worktreePaths, and src/relay/relay.ts:2340, :2568 and :4413 read it.
