...
const DECLARED: Record<string, string> = {
  ...
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7776.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:7393, src/relay/relay.ts:7913), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7510), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7827) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:7838), which is what ' +
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4518), where the
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2489-2495.
  // The cwd getter simply returns the option: src/relay/relay.ts:2190-2192.
  ...
  // worktreePaths, and src/relay/relay.ts:3427-3428, :4025 and :6594 read it.
