...
const DECLARED: Record<string, string> = {
  ...
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7894.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:7477, src/relay/relay.ts:8031), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:7612), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7945) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:7956), which is what ' +
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4527), where the
...
test('default run works in the run cwd and creates no worktree', async (t) => {
  ...
  // worktreePaths, and src/relay/relay.ts:3434-3435, :4032 and :6607 read it.
