...
interface CreateRecord {
   ...
   * for its whole life (src/repl/session.ts:265-277). This is the observation that would have
   * caught it.
   */
...
const DECLARED: Record<string, string> = {
  'implementer_unanswered -> advisor':
    ...
    'that raises it is at src/relay/relay.ts:7215.',
  ...
  "the console's /continue liveness guard samples by pause SCOPE, and no longer by rank":
    ...
    '(src/relay/relay.ts:6832, src/relay/relay.ts:7352), so every other pause reached that rank ' +
    ...
    'advisor_escalated sends to the ADVISOR (src/relay/relay.ts:6949), resuming ' +
    ...
    'could not be read (src/relay/relay.ts:7266) is conclave-scoped by design, yet the useful ' +
    ...
    'that seat’s liveness evidence from the halt site (src/relay/relay.ts:7277), which is what ' +
  ...
  'a mixed CPU sample is reported as mixed rather than as a working child (#83)':
    ...
    'file was relaxed; one citation moved with the code it pins (src/repl/session.ts:1530). ' +
  ...
  'a paused run keeps measuring the child, and its evidence says when it was measured (#101)':
    ...
    'has at src/repl/session.ts:361) and livenessRefreshMs/livenessRefreshLimit. No CLI flag ' +
  ...
  '--silence-timeout on both front-ends':
    ...
    'example of what that costs (src/repl/session.ts:265-277).',
 ...
 * call sites: the report is what `bin/conclave.ts:1803` prints, and the status record is what
 ...
 * `set('paused', { pause })` is the same call the console makes at src/repl/session.ts:1530
 ...
 * the same triggers reaching the same single halt site (src/relay/relay.ts:4154), where the
...
async function pausedStatusDocument(reason: PauseReason): Promise<unknown> {
  ...
  const recording = recordSession(relay, {
    ...
    front: 'session',
    startedAt: Date.now(),
    // Passed because the console always passes one (src/repl/session.ts:1212), so the status
...
test('default run works in the run cwd and creates no worktree', async () => {
  ...
  // The relay CLI passes process.cwd() as the run cwd: bin/conclave.ts:1711-1713.
  // The relay hands that same cwd to each participant adapter: src/relay/relay.ts:2262-2268.
  // The cwd getter simply returns the option: src/relay/relay.ts:1981-1983.
  ...
  // worktreePaths, and src/relay/relay.ts:3072-3073, :3670 and :6048 read it.
 ...
 *     its whole life (src/repl/session.ts:265-277), and a text-level pin cannot tell the
 *     difference.
