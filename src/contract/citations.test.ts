...
const CITED: Record<string, string> = {
  ...
  'src/relay/relay.ts:2060': 'worktreePaths(this.#opts.cwd)',
  'src/relay/relay.ts:2462': 'resolutionFor(p.subject, { rotationArmed: armed })',
  'src/relay/relay.ts:2639-2647': 'No rotation checks are configured',
  'src/relay/relay.ts:309': "onDegradation ?? 'candidate'",
  'src/relay/relay.ts:3411': 'worktreePaths(this.#opts.cwd)',
  'src/relay/relay.ts:3751': "subject: { reason: 'turn_incomplete', participant: lead.id }",
  'src/relay/relay.ts:4001': "subject: { reason: 'implementer_unanswered', participant: seat.id }",
  'src/relay/relay.ts:4134': "subject: { reason: 'turn_incomplete', participant: seat.id }",
