...
const CITED: Record<string, string> = {
  ...
  'bin/conclave.ts:1261': 'cwd: process.cwd(),',
  'bin/conclave.ts:1341': 'runReport(relay, { goal, outcome, startedAt: runStartedAt, build })',
  ...
  'bin/conclave.ts:1101-1104': "...extraArgs(flag('implementer-args', ''))",
  ...
  'src/repl/session.ts:861-874': 'progress.start(e.participant)',
  ...
  'src/repl/session.ts:655': 'escalates to you rather than being replaced',
  'src/repl/session.ts:818': 'logPath: runLogPath,',
  ...
  'src/repl/session.ts:1023': "recording.set('paused', { pause: run.pause })",
  ...
  'src/repl/session.ts:1650': 'so an in-place change like `superseded` reaches the file on the next one',
}
