...
export const CITED: Record<string, string> = {
  ...
  'bin/conclave.ts:1287': '{ ...s.status, alive: s.alive, abandoned: s.abandoned }',
  'bin/conclave.ts:2257-2259': "    const relay = await Relay.start({\n      registry,\n      cwd: process.cwd(),",
  'bin/conclave.ts:2359': 'runReport(relay, { goal, outcome, startedAt: runStartedAt, build })',
  ...
  'bin/conclave.ts:2042-2045': "...extraArgs(flag('implementer-args', ''))",
  ...
  'src/repl/session.ts:1491': 'progress.note(e.participant, () => labelFor(e.participant) ?? ev.tool)',
  ...
  'src/repl/session.ts:1375': 'logPath: runLogPath,',
  ...
  'src/repl/session.ts:1798': 'run.pause.refusal = { at: Date.now(), reason, ...(colour ? { liveness: colour } : {}) }',
