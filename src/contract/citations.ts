...
export const CITED: Record<string, string> = {
  ...
  'bin/conclave.ts:1276-1278': "    const relay = await Relay.start({\n      registry,\n      cwd: process.cwd(),",
  'bin/conclave.ts:1358': 'runReport(relay, { goal, outcome, startedAt: runStartedAt, build })',
  ...
  'bin/conclave.ts:1119-1122': "...extraArgs(flag('implementer-args', ''))",
  ...
  'src/relay/relay.ts:1730-1732': 'get cwd(): string',
  'src/relay/relay.ts:1999-2005': 'const ctx = { cwd, watchdogMs: this.#opts.turnWatchdogMs }',
  'src/relay/relay.ts:2671-2672': "this.#worktreesSeen.add(w)\n    return {",
  'src/relay/relay.ts:2732': 'NOT ARMED (no checks configured)',
  'src/relay/relay.ts:2965': 'if (this.#worktreesAtStart) for (const w of worktreePaths',
  'src/relay/relay.ts:3427': 'resolutionFor(p.subject, { rotationArmed: armed })',
  'src/relay/relay.ts:3925-3927': 'No rotation checks are configured',
  'src/relay/relay.ts:325': "onDegradation ?? 'candidate'",
  ...
  'src/relay/relay.ts:5121': 'this.#worktreesAtStart = worktreePaths(this.#opts.cwd)',
  'src/relay/relay.ts:5622': "subject: { reason: 'turn_incomplete', participant: lead.id }",
  ...
  'src/relay/relay.ts:5626': 'verdictOf: { participant: lead.id, endSeq: next.end.seq },',
  'src/relay/relay.ts:5738': 'The human has seen your escalation and asked you to continue.',
  ...
  'src/relay/relay.ts:5802': "reason: 'authority_conflict', workstream:",
  'src/relay/relay.ts:5886': "subject: { reason: 'implementer_unanswered', participant: seat.id }",
  'src/relay/relay.ts:5935-5937': "its report could not be read, so there is",
  ...
  'src/relay/relay.ts:5946-5948': "knowing whether the child is still writing changes what the operator does.",
  'src/relay/relay.ts:6019': "subject: { reason: 'turn_incomplete', participant: seat.id }",
  'src/relay/relay.ts:6023': 'verdictOf: { participant: seat.id, endSeq: current.seq },',
  ...
  'src/relay/run.ts:676': 'this.#pause.superseded = info',
  ...
  'src/repl/session.ts:325': 'liveness?: (pid: number) => Promise<ChildLiveness>',
  ...
  'src/repl/session.ts:870': 'escalates to you rather than being replaced',
  ...
  'src/repl/session.ts:1984': 'so an in-place change like `superseded` reaches the file on the next one',
  ...
  "src/repl/session.ts:2006": "if (word === '/rotate') {",
  "src/repl/session.ts:2039": "if (word === '/abort') {",
}
