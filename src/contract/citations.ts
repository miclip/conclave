...
export const CITED: Record<string, string> = {
  ...
  'src/relay/relay.ts:2236-2242': 'const ctx = { cwd, watchdogMs: this.#opts.turnWatchdogMs, idleMs: this.#opts.silenceWatchdogMs }',
  'src/relay/relay.ts:2970-2971': "this.#worktreesSeen.add(w)\n    return {",
  'src/relay/relay.ts:3071': 'NOT ARMED (no checks configured)',
  'src/relay/relay.ts:3568': 'if (this.#worktreesAtStart) for (const w of worktreePaths',
  'src/relay/relay.ts:4052': 'resolutionFor(p.subject, { rotationArmed: armed })',
  'src/relay/relay.ts:4657': 'No rotation checks are configured',
  ...
  'src/repl/session.ts:1156': 'progress.note(e.participant, label)',
  'src/relay/relay.ts:5932': 'this.#worktreesAtStart = worktreePaths(this.#opts.cwd)',
  'src/relay/relay.ts:6712': "subject: { reason: 'turn_incomplete', participant: lead.id }",
  ...
  'src/relay/relay.ts:6716': 'verdictOf: { participant: lead.id, endSeq: current.seq },',
  'src/relay/relay.ts:6833': 'The human has seen your escalation and asked you to continue.',
  ...
  'src/relay/relay.ts:6996': "reason: 'authority_conflict', workstream:",
  'src/relay/relay.ts:7095': "subject: { reason: 'implementer_unanswered', participant: seat.id }",
  'src/relay/relay.ts:7146': "its report could not be read, so there is",
  ...
  'src/relay/relay.ts:7157': "knowing whether the child is still writing changes what the operator does.",
  'src/relay/relay.ts:7228': "subject: { reason: 'turn_incomplete', participant: seat.id }",
  'src/relay/relay.ts:7232': 'verdictOf: { participant: seat.id, endSeq: current.seq },',
  ...
  'src/repl/session.ts:939': 'escalates to you rather than being replaced',
  'src/repl/session.ts:1090': 'logPath: runLogPath,',
  ...
  'src/repl/session.ts:1408': 'run.pause.refusal = { at: Date.now(), reason, ...(colour ? { liveness: colour } : {}) }',
  ...
  'src/repl/session.ts:2208': 'so an in-place change like `superseded` reaches the file on the next one',
  ...
  "src/repl/session.ts:2230": "if (word === '/rotate') {",
  "src/repl/session.ts:2263": "if (word === '/abort') {",
  ...
  'src/relay/relay.ts:4476': 'async #considerRotation(',
  'src/relay/relay.ts:4699-4703':
  ...
  'src/relay/relay.ts:4744-4748':
    ...
    "        ? this.#answeredByReplacement(impl, snap.compactionGeneration)\n" +
    "        : this.#acknowledge(impl, snap.compactionGeneration)",
  'src/relay/relay.ts:4770': 'return this.#acknowledge(impl, snap.compactionGeneration)',
  'src/relay/relay.ts:4801-4811':
    "        detail:\n" +
