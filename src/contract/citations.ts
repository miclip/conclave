...
export const CITED: Record<string, string> = {
  ...
  'bin/conclave.ts:1475-1477': "    const relay = await Relay.start({\n      registry,\n      cwd: process.cwd(),",
  'bin/conclave.ts:1567': 'runReport(relay, { goal, outcome, startedAt: runStartedAt, build })',
  ...
  'bin/conclave.ts:1259-1262': "...extraArgs(flag('implementer-args', ''))",
  ...
  'src/repl/session.ts:1200': 'progress.note(e.participant, label)',
  ...
  'src/repl/session.ts:983': 'escalates to you rather than being replaced',
  'src/repl/session.ts:1134': 'logPath: runLogPath,',
  ...
  'src/repl/session.ts:1452': 'run.pause.refusal = { at: Date.now(), reason, ...(colour ? { liveness: colour } : {}) }',
  ...
  'src/repl/session.ts:2272': 'so an in-place change like `superseded` reaches the file on the next one',
  ...
  "src/repl/session.ts:2294": "if (word === '/rotate') {",
  "src/repl/session.ts:2327": "if (word === '/abort') {",
  ...
  'src/repl/session.ts:1482': "inject(text, 'all')",
  'src/repl/session.ts:2390': 'inject(rest, { only: who })',
