...
export const CITED: Record<string, string> = {
  ...
  'bin/conclave.ts:2061-2063': "    const relay = await Relay.start({\n      registry,\n      cwd: process.cwd(),",
  'bin/conclave.ts:2160': 'runReport(relay, { goal, outcome, startedAt: runStartedAt, build })',
  ...
  'bin/conclave.ts:1845-1848': "...extraArgs(flag('implementer-args', ''))",
  ...
  'src/relay/relay.ts:2335-2337': 'get cwd(): string',
  'src/relay/relay.ts:2638-2644': 'const ctx = { cwd, watchdogMs: this.#opts.turnWatchdogMs, idleMs: this.#opts.silenceWatchdogMs }',
  'src/relay/relay.ts:3728-3729': "this.#worktreesSeen.add(w)\n    return {",
  'src/relay/relay.ts:3829': 'NOT ARMED (no checks configured)',
  'src/relay/relay.ts:4352': 'if (this.#worktreesAtStart) for (const w of worktreePaths',
  'src/relay/relay.ts:4853': 'resolutionFor(p.subject, { rotationArmed: armed })',
  'src/relay/relay.ts:5552': 'No rotation checks are configured',
  ...
  'src/relay/relay.ts:7039': 'this.#worktreesAtStart = worktreePaths(this.#opts.cwd)',
  'src/relay/relay.ts:7929': "subject: { reason: 'turn_incomplete', participant: lead.id }",
  ...
  'src/relay/relay.ts:7933': 'verdictOf: { participant: lead.id, endSeq: current.seq },',
  'src/relay/relay.ts:8234': 'The human has seen your escalation and asked you to continue.',
  ...
  'src/relay/relay.ts:8401': "reason: 'authority_conflict', workstream:",
  'src/relay/relay.ts:8516': "subject: { reason: 'implementer_unanswered', participant: seat.id }",
  'src/relay/relay.ts:8567': "its report could not be read, so there is",
  ...
  'src/relay/relay.ts:8578': "knowing whether the child is still writing changes what the operator does.",
  'src/relay/relay.ts:8649': "subject: { reason: 'turn_incomplete', participant: seat.id }",
  'src/relay/relay.ts:8653': 'verdictOf: { participant: seat.id, endSeq: current.seq },',
  ...
  'src/adapters/claude.ts:2404-2417': 'containedFallback: true,',
  ...
  'src/relay/relay.ts:5371': 'async #considerRotation(',
  'src/relay/relay.ts:5595-5599':
  ...
  'src/relay/relay.ts:5641-5645':
    ...
    "        ? this.#answeredByReplacement(impl, snap.compactionGeneration)\n" +
    "        : this.#acknowledge(impl, snap.compactionGeneration)",
  'src/relay/relay.ts:5667': 'return this.#acknowledge(impl, snap.compactionGeneration)',
  'src/relay/relay.ts:5699-5709':
    "        detail:\n" +
