 ...
 * looks: it is set at exactly two halt sites, both `turn_incomplete`
  * (`src/relay/relay.ts:7526` and `src/relay/relay.ts:8080`). So FOUR of the five seat-scoped
 ...
 * from a rank. Worse than useless on one of them: resuming an `advisor_escalated` pause sends
  * to the ADVISOR (`src/relay/relay.ts:7665`), so the fallback measured children that were not
 * about to be sent to at all.
 *
 * What that gives up, stated rather than discovered: the `advisor_escalated` halt raised when a
  * seat's turn completed and its report could not be read (`src/relay/relay.ts:7998`) is
 ...
 * of being the only implementer. It is not sampled now. The pause still carries its own
  * liveness EVIDENCE from the halt site (`src/relay/relay.ts:8009`), which is what the operator
 * reads;
...
export async function runSession(opts: SessionOptions): Promise<number> {
  ...
  const relay = await Relay.start({
    ...
    ...(opts.ceilings ? { ceilings: opts.ceilings } : {}),
    // The same reading of the same file the launch arguments came from, not a second one: a
  ...
  const resumeRun = async (runOpts: { force?: boolean } = {}): Promise<void> => {
    ...
    //
    // Read off the SEAT'S OWN EVENTS, not off the pause, and that is a correction to the first
    // cut of this fix rather than a detail. Keying it on `pause.superseded` looked tighter and
    // was wrong: a `compaction` revision is also the degradation signal, so on a run with no
    // rotation checks the seat's bare withdrawal raises a `rotation_candidate` pause FIRST --
    // a pause with no `verdictOf` and no supersession, sitting in front of an operator whose
    // seat's turn is open only because a record was deleted. The pause-keyed version refused
    // that one forever, which is #66 again wearing a different reason. Found by the test, not
    // by reading.
    //
    // So the question is asked of the thing it is about: is THIS seat's open turn an
    // observation or a deleted record? Two conditions, and each is a case it must not wave
    // through:
    //
    //   - the open turn must be the one a withdrawal REOPENED (`ActiveTurn.withdrawn`), not
    //     one that started after it. A `turn_start` is an observation -- the adapters drop the
    //     transcript view's lifecycle events and forward only content, so any `turn_start`
    //     here came from a hook -- and a child seen starting a turn is a child to refuse,
    //     whatever was withdrawn beforehand;
    //   - and no replacement verdict, which this gets for free rather than by testing for it:
    //     a replacement IS a `turn_end`, so it closes the turn and there is no open turn left
    //     to bypass. `timed_out` -- the replacement that says the turn is still running -- is
    //     the case that matters, and it re-arms the guard on the next reading by itself.
    //
    ...
    if (runOpts.force && run.pause) {
      ...
      for (const child of children) {
        ...
        if (turn.withdrawn) {
          ...
          write(`  the verdict that closed its turn was taken back (${turn.withdrawn.reason}) and nothing replaced it.`)
          write(`  the turn is open only because that record was deleted, not because a turn was seen to start,`)
  ...
  const supervise = async (handle: RunHandle): Promise<void> => {
    for (;;) {
      ...
      // The console has gone, and this loop is waking up during its teardown rather than in
      // front of an operator.
      //
      // It could not happen before #142: `relay.stop()` left the handle unsettled, so a console
      // that exited with a run still in flight parked this `await` forever and nothing below ran.
      // Now `stop()` settles, and everything below is addressed to a session that no longer
      // exists -- `write` draws on a closed screen, `prompt()` calls a closed readline, and
      // `recording.set('starting')` would publish `starting` OVER the `ended` the teardown had
      // just written. That last one is the damaging one: `conclave status` would report a
      // session waiting for its next instruction with nobody there, and `conclave events
      // --follow` stops on `ended` and would never stop at all.
      //
      // The same shape as the guard in `refreshPrompt`, for the same reason it gives: a run torn
      // down mid-turn keeps resolving after readline has closed. `runEnded` is skipped with the
      // rest deliberately -- the piped form races it before this point, so by here it has either
      // fired or been overtaken by the `close` that set `done`.
      if (done) return
      if (s.kind === 'ended') {
        ...
        // What `--checks` actually reached, on both front-ends for the same reason the
        // rotation summary is on both (#153).
        const integration = relay.integrationSummary()
        if (integration !== undefined) write(summaryLine('===', integration, width))
