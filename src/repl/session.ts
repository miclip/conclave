 ...
 * (`src/relay/relay.ts:4905` and `src/relay/relay.ts:5289`). So FOUR of the five seat-scoped
 ...
 * to the ADVISOR (`src/relay/relay.ts:5004`), so the fallback measured children that were not
 * about to be sent to at all.
 *
 * What that gives up, stated rather than discovered: the `advisor_escalated` halt raised when a
 * seat's turn completed and its report could not be read (`src/relay/relay.ts:5201`) is
 ...
 * liveness EVIDENCE from the halt site (`src/relay/relay.ts:5215`), which is what the operator
 * reads;
