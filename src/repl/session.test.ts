...
test('a participant-scoped pause samples that seat and no other, at every reason that carries one', () => {
  ...
  // (src/relay/relay.ts:7618). A rank scan for implementers sampled the wrong child here too.
  assert.deepEqual(
...
test('a conclave- or workstream-scoped pause samples nobody, with no fall back to rank', () => {
  ...
  // (src/relay/relay.ts:7757), so measuring implementer children was never the question; and
  ...
  // is named after the seat carrying the instruction (src/relay/relay.ts:7924), which is exactly
...
test('a rotation_candidate pause on one seat resumes while the OTHER seat is genuinely mid-turn', async (t) => {
  ...
  // sites, both turn_incomplete (src/relay/relay.ts:7622, src/relay/relay.ts:8176) -- so under
  ...
  const running = runSession({
    ...
    // (src/relay/relay.ts:5270). A command that exits 0 immediately: what the checks DO is
