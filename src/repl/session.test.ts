...
test('a force into a held turn ends peer_busy with turnsCompleted 0', async () => {
  // The death-side population of #125: the same `/continue force` command, the same record
  // shape, but the run dies in the only attributable window. The increment-4 test
  // ('a force that ends the run gets a distance-qualified followedBy record') ended normally
  // with followedBy.turnsCompleted >= 1 and a non-busy outcome; here the entry reads
  // followedBy.outcome === 'peer_busy' and followedBy.turnsCompleted === 0. No causal label
  // separates a justified force from a fatal one -- the distance facts do.
  const dir = repo()
  ...
  // second work turn is the one the forced resume sends into and which never ends.
  impl.endTurn = { index: 1, verdict: TIMED_OUT, withdraw: 'no_replacement' }
  impl.holdTurn = 2
  impl.childPid = 1
  const out = collect()
  ...
  assert.equal(
    entry.followedBy!.turnsCompleted,
    0,
    'no turn completed after the force -- the attributable window',
  )
  assert.ok(
