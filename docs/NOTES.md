...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4942`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5165-5169, :5210-5214, :5236, :5267-5277, :5281-5284`): runs only after a completed implementer turn (`src/relay/relay.ts:7933`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:641-647`); Claude's view is created at boot (`src/adapters/claude.ts:1255-1259`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2496`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1412` and `src/relay/relay.ts:4098` consume only `snap.turns`; unaffected.
