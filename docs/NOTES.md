...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4555`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:4778-4782, :4823-4827, :4849, :4880-4890, :4894-4897`): runs only after a completed implementer turn (`src/relay/relay.ts:7335`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:605-611`); Claude's view is created at boot (`src/adapters/claude.ts:1084-1088`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2246`) and `assess()` would see no delta.
