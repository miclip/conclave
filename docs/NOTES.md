...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4720`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:4943-4947, :4988-4992, :5014, :5045-5055, :5059-5062`): runs only after a completed implementer turn (`src/relay/relay.ts:7561`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:620-626`); Claude's view is created at boot (`src/adapters/claude.ts:1183-1187`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2395`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1412` and `src/relay/relay.ts:3885` consume only `snap.turns`; unaffected.
