...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:5089`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5313-5317, :5359-5363, :5385, :5417-5427, :5432-5435`): runs only after a completed implementer turn (`src/relay/relay.ts:8196`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:666-672`); Claude's view is created at boot (`src/adapters/claude.ts:1359-1363`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2521`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1710` and `src/relay/relay.ts:4240` consume only `snap.turns`; unaffected.
