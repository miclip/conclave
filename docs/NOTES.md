...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4381`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:4604-4608, :4649-4653, :4675, :4706-4716, :4720-4723`): runs only after a completed implementer turn (`src/relay/relay.ts:7111`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:556-562`); Claude's view is created at boot (`src/adapters/claude.ts:1025-1029`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2165`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1408` and `src/relay/relay.ts:3546` consume only `snap.turns`; unaffected.
