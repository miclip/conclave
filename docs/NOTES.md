...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4716`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:4939-4943, :4984-4988, :5010, :5041-5051, :5055-5058`): runs only after a completed implementer turn (`src/relay/relay.ts:7536`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:620-626`); Claude's view is created at boot (`src/adapters/claude.ts:1183-1187`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2391`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1412` and `src/relay/relay.ts:3881` consume only `snap.turns`; unaffected.
