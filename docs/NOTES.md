...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4901`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5124-5128, :5169-5173, :5195, :5226-5236, :5240-5243`): runs only after a completed implementer turn (`src/relay/relay.ts:7863`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:620-626`); Claude's view is created at boot (`src/adapters/claude.ts:1183-1187`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2485`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1412` and `src/relay/relay.ts:4057` consume only `snap.turns`; unaffected.
