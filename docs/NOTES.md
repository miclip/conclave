...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:5784`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:6008-6012, :6054-6058, :6080, :6112-6122, :6127-6130`): runs only after a completed implementer turn (`src/relay/relay.ts:9103`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:709-715`); Claude's view is created at boot (`src/adapters/claude.ts:1473-1477`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2768`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1794` and `src/relay/relay.ts:4730` consume only `snap.turns`; unaffected.
