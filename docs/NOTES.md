...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:5394`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5618-5622, :5664-5668, :5690, :5722-5732, :5737-5740`): runs only after a completed implementer turn (`src/relay/relay.ts:8696`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:677-683`); Claude's view is created at boot (`src/adapters/claude.ts:1399-1403`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2652`) and `assess()` would see no delta.
