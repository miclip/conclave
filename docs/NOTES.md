...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:5381`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5605-5609, :5651-5655, :5677, :5709-5719, :5724-5727`): runs only after a completed implementer turn (`src/relay/relay.ts:8683`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:677-683`); Claude's view is created at boot (`src/adapters/claude.ts:1399-1403`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2652`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1794` and `src/relay/relay.ts:4437` consume only `snap.turns`; unaffected.
