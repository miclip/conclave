...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:4391`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:4614-4618, :4659-4663, :4685, :4716-4726, :4730-4733`): runs only after a completed implementer turn (`src/relay/relay.ts:7121`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:556-562`); Claude's view is created at boot (`src/adapters/claude.ts:1025-1029`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2165`) and `assess()` would see no delta.
   ...
   - Snapshot reads at `src/workspace/sessionRecord.ts:1408` and `src/relay/relay.ts:3556` consume only `snap.turns`; unaffected.
