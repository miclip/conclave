...
2. Every consumer of a snapshot's `compactionGeneration` on main, and whether a pre-view snapshot can reach it:
   ...
   - `#considerRotation` (`src/relay/relay.ts:5545`, plus downstream `#acknowledge`/`#answeredByReplacement` at `src/relay/relay.ts:5769-5773, :5815-5819, :5841, :5873-5883, :5888-5891`): runs only after a completed implementer turn (`src/relay/relay.ts:8862`). A completed first turn means Codex's `SessionStart` hook already delivered `transcript_path` (`src/adapters/codex.ts:712-718`); Claude's view is created at boot (`src/adapters/claude.ts:1473-1477`). Cannot observe pre-view — and would be inert anyway, since `baselineGeneration` starts at 0 (`src/relay/relay.ts:2685`) and `assess()` would see no delta.
